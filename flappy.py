...
def showGameOverScreen(crashInfo):
    ...
    for select in range(total_models):
        fitness[select] = float(fitness[select])/total_fitness
        temp.append((fitness[select],select))
    temp.sort()
    for select in range(int(total_models/2)):
        idx1 = randint(30,total_models-1);#select a model among the 20 best models
        idx2 = randint(30,total_models-1);#select a model among the 20 best models
        new_weights1 = model_crossover(temp[idx1][1], temp[idx2][1])# crossover by randomly selecting among the 20 best models
